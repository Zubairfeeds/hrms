...
def create_salary_slips_for_employees(employees, args, publish_progress=True):
    try:
        ...
        for emp in employees:
            # Find the employee entry in the Payroll Entry child table
            employee_entry = next(
                (e for e in payroll_entry.employees if e.employee == emp), None
            )
            
            if employee_entry:
                # Get the gross monthly salary from the Payroll Entry employee entry
                gross_monthly_salary = employee_entry.gross_monthly_salary
                
                # Update the arguments with the gross monthly salary
                args.update({
                    "doctype": "Salary Slip",
                    "employee": emp,
                ...
                # Create and insert the Salary Slip document
