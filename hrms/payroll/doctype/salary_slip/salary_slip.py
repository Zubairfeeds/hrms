...
class SalarySlip(TransactionBase):
	def __init__(self, *args, **kwargs):
		...
		self.series = "Pay Slip/{0}/.#####".format(self.employee)
...
# *** Custom Code 16-2-2024 till end
	

@frappe.whitelist()
def get_gross_salary(employee):
    salary_structure_assignment = frappe.get_value(
        'Salary Structure Assignment', {'employee': employee, "docstatus": 1}, 'gross_monthly_salary'
...
# def set_gross_salary_in_salary_slip(doc, method):
#     salary_slips = frappe.get_all(
#         "Salary Slip", filters={"docstatus": 0}, fields=["name", "employee"]
#     )

#     for salary_slip in salary_slips:
#         gross_salary = get_gross_salary(salary_slip.employee)

#         salary_slip_doc = frappe.get_doc("Salary Slip", salary_slip.name)
#         salary_slip_doc.gross_monthly_salary = gross_salary
...
# def set_gross_salary_in_salary_slip(doc, method):
#     salary_slips = frappe.get_all(
#         # "Salary Slip", filters={"docstatus": 0, "posting_date": today()}, fields=["name", "employee"]
#         "Salary Slip", filters={"docstatus":  0}, fields=["name", "employee"]
...
#         if gross_salary:
#             frappe.db.set_value("Salary Slip", salary_slip.name, "gross_monthly_salary", gross_salary)
            # frappe.msgprint(f"Salary Slip created successfully for {salary_slip.name}")
        # else:
        #     frappe.msgprint(f"No gross salary found for employee {salary_slip.employee}")
