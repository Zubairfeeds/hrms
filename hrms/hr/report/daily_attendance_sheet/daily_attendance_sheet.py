...
def execute(filters: Optional[Filters] = None) -> Tuple:
	...
	# if not (filters.form_date and filters.to_date):
	...
	if not attendance_map:
		...
		columns = get_columns(filters, summarized_view=filters.get("summarized_view", False))
...
def get_columns_for_days(filters: Filters) -> List[Dict]:
    ...
    total_days = (to_date - from_date).days + 1  # Total days between from_date and to_date

    days = []

    for i in range(total_days):
        date = from_date + timedelta(days=i)
        # Get the day of the week and its abbreviation
        weekday = day_abbr[date.weekday()]
        # Set days as 1 Mon, 2 Tue, 3 Wed, ...
        label = "{} {}".format(date.day, weekday)
        days.append({"label": label, "fieldtype": "Data", "fieldname": date.day, "width": 65})
...
def get_attendance_map(filters: Filters) -> Dict:
    """Returns a dictionary of employee wise attendance map as per shifts for all the days within the specified date range."""
...
def get_holiday_map(filters: Filters) -> Dict[str, List[Dict]]:
    """
    Returns a dict of holidays falling in the filter date range
...
#                 row = {"employee": employee, "employee_name": details.employee_name, "shift": shift}
#                 for day in range(1, total_days + 1):
    ...
    for holiday in holidays:
        if day == holiday.get("day_of_month"):
            if holiday.get("weekly_off"):
                ...
                status = "Holiday"
            break
    return status or ""  # Return an empty string if status is None
