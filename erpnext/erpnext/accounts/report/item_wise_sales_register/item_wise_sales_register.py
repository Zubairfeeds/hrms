...
def _execute(filters=None, additional_table_columns=None, additional_conditions=None):
	...
	for d in item_list:
		...
		row = {
			...
			"gross_discount_amount": d.gross_discount_amount,
			"qty": d.qty,
			"price_list_rate": d.price_list_rate,
			"price_list_amount": d.price_list_amount,
			"invoice": d.parent,
...
def get_columns(additional_table_columns, filters):
	...
	columns.extend(
		[
			...
			{"label": _("Price List Amount"), "fieldname": "price_list_amount", "fieldtype": "Currency", "width": 120},
			{"label": _("Discount Amount"), "fieldname": "gross_discount_amount", "fieldtype": "Float", "width": 120},
...
def get_conditions(filters, additional_conditions=None):
	...
	for opts in (
		...
		("site", " and `tabSales Invoice`.site = %(site)s"),
...
def get_items(filters, additional_query_columns, additional_conditions=None):
	...
	return frappe.db.sql(
		...
		select
			...
			`tabSales Invoice`.customer, `tabSales Invoice`.remarks,`tabSales Invoice`.site,
			`tabSales Invoice`.territory, `tabSales Invoice`.company, `tabSales Invoice`.base_net_total,
			`tabSales Invoice Item`.project,
			`tabSales Invoice Item`.item_code, `tabSales Invoice Item`.description,
			`tabSales Invoice Item`.gross_discount_amount,
			`tabSales Invoice Item`.qty,
			`tabSales Invoice Item`.price_list_rate,
			`tabSales Invoice Item`.price_list_amount,
	...
	if prev_group_by_value != item.get(group_by_field, ""):
		...
		total_row_map.setdefault(
			item.get(group_by_field, ""),
			{
				...
				"stock_qty": 0.0,
				"gross_discount_amount": 0.0,
				"qty": 0.0,
				# "price_list_rate": 0.0,
				"price_list_amount": 0.0,
				"rate":0.0,
				"amount": 0.0,
				"bold": 1,
				"total_tax": 0.0,
...
def add_sub_total_row(item, total_row_map, group_by_value, tax_columns):
	...
	# total_row["gross_discount_amount"] += item["gross_discount_amount"]
	total_row["qty"] += item["qty"]
	# total_row["price_list_rate"] += item["price_list_rate"]
	# total_row["price_list_amount"] += item["price_list_amount"]
	# total_row["rate"] += item["rate"]
