...
def get_data(filters, show_party_name):
	...
	for party in parties:
		...
		# Quantity
		debit_qty, credit_qty = get_quantity_for_party(filters, party.name)
		row.update({"debit_qty": debit_qty, "credit_qty": credit_qty})
...
def get_quantity_for_party(filters, party_name):
    # Assuming the quantity field is in the GL Entry doctype
    sales_invoices = frappe.get_all(
        "Sales Invoice",
        filters={"customer": party_name},  # Assuming 'customer' is the field representing the party in Sales Invoice
        fields=["name"],
    )

    debit_qty, credit_qty = 0, 0  # Initialize both quantities to 0
    ...
    debit_qty = sum(entry.qty for entry in invoice_items if entry.debit)
    credit_qty = sum(entry.qty for entry in invoice_items if entry.credit)

    return debit_qty, credit_qty
...
def get_columns(filters, show_party_name):
	columns = [
		...
		{
  			"fieldname": "debit_qty",
    			"label": _("Debit Quantity"),
    			"fieldtype": "Float",
    			"width": 120,
		},
		{
    			"fieldname": "credit_qty",
    			"label": _("Credit Quantity"),
    			"fieldtype": "Float",
    			"width": 120,
		},
	]
