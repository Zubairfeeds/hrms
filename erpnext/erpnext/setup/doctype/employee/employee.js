...
frappe.ui.form.on('Employee', {
		refresh: function(frm) {
			// Add a button to create PF Member if the employee is eligible
			if (frm.doc.yes == 1) {
				frm.add_custom_button(__('Create PF Member'), function() {
					frappe.call({
				 		method: 'erpnext.setup.doctype.employee.employee.create_pf_member',
						args: {
						...
						callback: function(r) {
							...
							// 	frappe.set_route('Form', 'PF Member', r.message.name);
							// }
							if(r.message){
								frm.refresh();
							}
