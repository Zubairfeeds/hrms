...
@frappe.whitelist()
def create_pf_member(employee):
    try:
        # Check if a PF Member record already exists for this employee
        if frappe.db.exists("PF Member", {"employee": employee}):
            frappe.msgprint(_("PF Member record already exists for this employee."))
            return

        # Retrieve the Employee document
        emp_doc = frappe.get_doc("Employee", employee)

        # Create new PF Member record
        new_pf_member = frappe.get_doc({
            "doctype": "PF Member",
            "employee": emp_doc.name,
			"employee_number": emp_doc.name,
            "gender": emp_doc.gender,
            "date_of_joining": emp_doc.date_of_joining,
            "branch": emp_doc.branch,
			"company": "Zubair Feeds Provident Fund Trust"
            # Add other fields as per your DocType definition
        })

        new_pf_member.insert(ignore_permissions=True)
        frappe.msgprint(_("PF Member record created successfully."))
    
    except Exception as e:
        frappe.msgprint(_("Failed to create PF Member record: {0}").format(str(e)))
